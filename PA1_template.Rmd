---
title: "Course Project - Reproducible Reasearch"
...
has been created in order to convert the date + interval columns to posXIct.
Also, where steps data is not available, rows have been removed.
...
```
2. Make a histogram of the total steps taken per day
...
The mean is `r round(steps_mean,1)` steps per day and the median is \
`r steps_median` 
steps per day.


## What is the average daily activity pattern?
1. Make a time series plot of the 5 minute interval (x axis) vs the average
...
The interval that contains the maximum number of steps as average is
`r maximum_interval` to `r as.numeric(maximum_interval)+5`
...
The missing data can be inputted as the average for that interval and that
day of the week
...
```
After missing values are imputed the mean is `r round(steps_mean_2,1)` steps per
day and the median is `r steps_median_2` steps. This means the mean is increasing 
by `r mean_diff` per day whereas the median
is increasing by `r median_diff`.
